        ...
        for (int j = 0; j < r; j++) {
            for (int p = i * 8; p < (i + 1) * 8 && k < (j + 1) * 8 * (i + 1);
                ...
                if (k == l) {
                    ...
                    stop = 1;
                    break;
                } else // first alpha -> type // next 8 bits according to
                       // sequence rajni->!2QA
                {
                    // int num=(int)s[k]-48;
                    ...
                    for (int q = j * 8; q < (j + 1) * 8; q++) {
                        ...
                        if (q > j * 8) {
                            ...
                            arr[q - (j * 8) - 1] =
                                m[p][q]; // ascii value //stored in  reverse
                                         // like 6 : 0 1 1 0 0 0 0
                        }
                    }
