...
string encript_word(string &s) {
    ...
    for (int i = 0; i < r; i++) {
        for (int j = 0; j < r; j++) {
            ...
            // {
            for (int p = i * 8; p < (i + 1) * 8 && k < (j + 1) * 8 * (i + 1);
                 p++, k++) {
                if (k == l) {
                    stop = 1;
                    // cout<<"hi\n";
                    ...
                    if (num >= 0 && num <= 9) {
                        ...
                        for (int q = j * 8; q < (j + 1) * 8; q++) {
                            if (q == j * 8) {
                                ...
                                m[p][q] =
                                    b[q - j * 8 - 1]; // stored in  reverse like
                                                      // 6 : 0 1 1 0 0 0 0
                            }
                        }
                    } else if (isupper(s[k])) {
...
void read_word_by_word(string filename) {
    ...
    while (getline(file, line)) {
        ...
        f << encript_word(eol) << endl;
        vector<string> tem = rand_str();
        string num = to_string(tem.size());
        f << encript_word(num) << endl;
        for (auto i : tem) {
            f << encript_word(i) << endl;
        }
    }
    file.close();
}
